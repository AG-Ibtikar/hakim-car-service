...
NODE_VERSION="18"
SERVER_IP="147.93.72.229"
GITHUB_REPO="https://github.com/your-username/hakim-car-service.git"  # Update this with your actual GitHub repo URL
...
# Clone and setup application from GitHub
setup_application() {
    log "Setting up application from GitHub..."
    ...
    sudo -u $APP_USER git clone $GITHUB_REPO . || {
        error "Failed to clone repository from GitHub. Please check the repository URL and access permissions."
    }
...
# JWT
JWT_SECRET="hakim-jwt-secret-2024-production-key-change-this"
JWT_EXPIRES_IN="7d"
...
# CORS
CORS_ORIGIN="http://$SERVER_IP:$FRONTEND_PORT,http://$SERVER_IP,http://localhost:$FRONTEND_PORT"
...
NEXTAUTH_SECRET="hakim-nextauth-secret-2024-production-key"
...
server {
    listen 80;
    server_name $SERVER_IP;  # Using IP address
...
display_info() {
    ...
    echo -e "Backend URL: ${BLUE}http://$SERVER_IP:$BACKEND_PORT${NC}"
    echo -e "Frontend URL: ${BLUE}http://$SERVER_IP:$FRONTEND_PORT${NC}"
    echo -e "Main Application URL: ${BLUE}http://$SERVER_IP${NC}"
    ...
    echo "1. Update the GITHUB_REPO variable in this script with your actual repository URL"
    echo "2. Update environment variables with production values (Firebase, AWS, etc.)"
    echo "3. Configure domain name if you have one"
    echo "4. Setup SSL certificate with Certbot if needed"
    ...
    echo "Access application: http://$SERVER_IP"
}

# Main deployment function
main() {
    log "Starting Hakim Car Service deployment on Hostinger VPS ($SERVER_IP)..."
